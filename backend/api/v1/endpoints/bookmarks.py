...
from schemas import bookmark as schemas
...
@router.get("/bookmarks", response_model=schemas.BookmarkListResponse, summary="获取收藏列表")
...
async def check_batch_bookmarked(
    request: schemas.BookmarkCheckRequest,
    current_user: User = Depends(get_current_user),
    redis: Redis = Depends(get_redis)
):
    """
